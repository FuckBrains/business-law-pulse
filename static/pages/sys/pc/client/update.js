...
define([
    ...
    'text!pages/sys/pc/client/ejs/update.html',
    'text!pages/sys/pc/client/ejs/pick_list.html',
    'bootstrap-multiselect',
    'ueditor','ueditor-lang',
    ], function(ejs_loading, ejs_pagination, ejs_update, ejs_pick_list) {
    ...
    self.render_components = function() {
        ...
        UE.getEditor('note_editor', {
            ...
            this.setContent(self.client.note);
        });
        ...
        UE.getEditor('note_cn_editor', {
            ...
            this.setContent(self.client.note_cn);
        });
        ...
        UE.getEditor('raw_editor', {
            ...
            this.setContent(self.client.raw);
        });
    };
    ...
    $('.site-main').on('click','.update-client-btn',function(event) {
        ...
        validator.form();
        if (validator.valid()) {
            $('.update-client-btn').button('loading');
            CORE.ajax.post({
                url: '/law/client/' + self.client.id + '/update',
                headers: {
                ...
                complete: function(xhr) {
                    $('.update-client-btn').button('reset');
                }
            });
        }
    });
    ...
    self.search_client = function() {
        ...
        setTimeout(function() {
            CORE.ajax.post({
                ...
                success: function(result) {
                    $('.client-list').html(new EJS({ text: ejs.pick_list }).render({ clients: result.data.clients }));
    ...
    return function(ctx,next) {
        self.load_config(function() {
            CORE.ajax.post({
                url: '/law/client/' + ctx.params.client + '/detail',
                headers: {
                    'x-admin-id': CORE.admin.id,
                    'x-admin-token': CORE.admin.token,
                },
                success: function(result) {
                    self.client = result.data.client;
