...
define([
    ...
    'text!pages/sys/pc/client/ejs/query.html',
    'text!pages/sys/pc/client/ejs/list.html',
    ...
    $('.site-main').on('click','.remove-client-btn',function(event) {
        if (!confirm('Are you sure of removing this record ?')) return false;

        var tr = $(event.currentTarget).parents('tr');

        CORE.ajax.post({
            url: '/law/client/' + tr.data('client-id') + '/remove',
            headers: {
    ...
    self.search_client = function() {
        ...
        $('.client-list').html(new EJS({ text: ejs.loading }).render({ padding: '100px', font: '50px' }));
        $('.client-pagination').html('');
        ...
        setTimeout(function() {
            CORE.ajax.post({
                ...
                success: function(result) {
                    $('.client-list').html(new EJS({ text: ejs.list }).render({ clients: result.data.clients }));
